...
| CORS | Allowlist only: `https://rtu-qr-tracker.quadreal-rpiwin.workers.dev` (desktop browsers), `capacitor://localhost` (iOS shell), `https://localhost` (Android shell, per `androidScheme: 'https'`), plus the legacy shells' `https://appassets.androidplatform.net` and `rtuapp://app`. The tracker origin is easy to forget because the shells load the same `index.html` from a different origin — leave it out and desktop sign-in fails at the preflight, before the password is checked. Responses include `Vary: Origin` (no `*`). Live reload (`npm run dev:ios`) serves from a LAN address that is deliberately **not** allowlisted — sign-in and upload only work in a normal build. |

/**
 * Local live-reload for Capacitor.
 *
 *   npm run dev            → desktop browser
 *   npm run dev:android    → phone over USB (adb reverse) or Wi-Fi
 *   npm run dev:ios        → iPhone over Wi-Fi (Mac + Xcode)
 *
 * Serves the repo root. Edits to index.html / piexif.js reload instantly.
 * native-bridge.js (ESM source) is rebundled to an IIFE on each change and
 * served at /native-bridge.js so the classic <script> tag keeps working.
 */
...
const PORT = Number(process.env.DEV_PORT || 3333);
...
// Rebuild bridge on source change (avoid watching the ESM path as a bs file,
// since the source is not what the browser loads).
